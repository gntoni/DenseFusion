...
cinfo_file = open('{0}/camera_info.yml'.format(dataset_config_dir), 'r')
...
for i, data in enumerate(testdataloader, 0):
    ...
    if i%250 == 0:
        for pt3d in pred:
            pt2d = cam_K.dot(pt3d)
            pt2d = pt2d[:2]/pt2d[2]
            plt.plot(pt2d[0],pt2d[1], marker='o', markersize=0.8, color="red", alpha=0.4)

        for pt3d in target:
            pt2d = cam_K.dot(pt3d)
            pt2d = pt2d[:2]/pt2d[2]
            plt.plot(pt2d[0],pt2d[1], marker='o', markersize=0.6, color="green", alpha=0.4)
